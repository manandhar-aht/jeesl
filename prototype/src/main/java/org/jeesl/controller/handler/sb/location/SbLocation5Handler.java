...
public class SbLocation5Handler <L1 extends EjbWithId, L2 extends EjbWithId, L3 extends EjbWithId, L4 extends EjbWithId, L5 extends EjbWithId> extends SbLocation4Handler<L1,L2,L3,L4>
{
	...
	protected final Set<L5> allowedL5;
	protected L5 l5; public L5 getL5(){return l5;} public void setL5(L5 l5){this.l5 = l5;}
	private boolean includeAlll5; public void setIncludeAlll5(boolean includeAlll5) {this.includeAlll5 = includeAlll5;}
	public SbLocation5Handler(JeeslLocation5Cache<L1,L2,L3,L4,L5> cache5, JeeslLocation5Store<L1,L2,L3,L4,L5> store5)
	{
		super(cache5,store5);
		this.cache5=cache5;
		this.store5=store5;
		list5 = new ArrayList<L5>();
		allowedL5 = new HashSet<L5>();
		includeAlll5 = false;
	}
	...
	protected void addAllowedL5(List<L5> list)
	{
		for(L5 ejb : list)
		{
			if(!allowedL5.contains(ejb)) {allowedL5.add(ejb);}
			if(!allowedL4.contains(getParent5(ejb)))
			{
	...
	public void ui5(L5 ejb) {select5(ejb,HierarchicalLocationUpdateParameter.build(false,true,true,true));}
	public void select5(L5 ejb, HierarchicalLocationUpdateParameter hlup)
	{
		...
		store5.setL5(ejb);
		clearL6List();
		if(hlup.isFillParent()) {select4(getParent5(l5),hlup.withSelectChild(false));}
		if(hlup.isFillChilds()) {fillL6List();}
		if(hlup.isSelectChild()) {selectDefaultL6(hlup.withFillParent(false));}
	...
	@Override protected void fillL5List()
	{
		...
		{
			if(viewIsGlobal || includeAlll5 || allowedL5.contains(ejb)) {list5.add(ejb);}
		}
	}
	@Override protected void selectDefaultL5(HierarchicalLocationUpdateParameter hlup)
	{
		reset(5);
		if(!list5.isEmpty()) {select5(list5.get(0),hlup.withFillParent(false));}
	}
