					...
					implements Serializable,SbSingleBean
{
	...
	public AbstractAdminSurveyTemplateBean(SurveyTemplateFactoryBuilder<L,D,SURVEY,SS,SCHEME,TEMPLATE,VERSION,TS,TC,SECTION,QUESTION,QE,SCORE,UNIT,ANSWER,MATRIX,DATA,OPTIONS,OPTION> fbTemplate,
	...
	{
		template = fCore.load(template,false,false);
	...
	protected <E extends Enum<E>> void initTemplate(boolean withVersion, E statusCode)
	{
		if(sbhCategory.isSelected() && version!=null)
		{
			try
			{
				TS status = fCore.fByCode(cTs,statusCode);
				template = fCore.fcSurveyTemplate(sbhCategory.getSelection(),version,status,nestedVersion);
