...
public class JeeslTreeFigureFactory
{
		...
		Figures build(String localeCode, JeeslPivotFactory<L,D,A> pivotFactory, int lvl, List<A> aggregations, JeeslPivotAggregator dpa, List<EjbWithId> parents, TRANSFORMATION transformation)
	{
		...
		{
			data.getFigures().addAll(JeeslTreeFigureFactory.aggregationLevel(localeCode,pivotFactory,0,aggregations,dpa,new ArrayList<EjbWithId>()));
		}
	...
	private static <L extends UtilsLang, D extends UtilsDescription, A extends UtilsStatus<A,L,D>>
		List<Figures> aggregationLevel(String localeCode, JeeslPivotFactory<L,D,A> pivotFactory, int lvl, List<A> aggregations, JeeslPivotAggregator dpa, List<EjbWithId> parents)
	{
		...
		{			
			...
			if(aggregations.size()>lvl+1)
			{
				figures.getFinance().addAll(pivotFactory.buildFinance(dpa,path));
				figures.getFigures().addAll(aggregationLevel(localeCode,pivotFactory,lvl+1,aggregations,dpa,path));
			}
			else if(aggregations.size()==lvl+1)
			{
				figures.getFinance().addAll(pivotFactory.buildFinance(dpa,path));
			}
